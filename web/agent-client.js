...
// sdk/node_modules/viem/_esm/utils/unit/Value.js
...
// sdk/node_modules/viem/_esm/accounts/generatePrivateKey.js
init_secp256k1();
init_toHex();
...
var SIG_TYPE_AGENT = 1;
var SIG_TYPE_ESCALATED = 2;
...
function importAgentKey(privateKeyHex) {
...
function deriveAgentAddress(privateKeyHex) {
  const account = privateKeyToAccount(privateKeyHex);
  return account.address;
}
async function signUserOpHash(privateKeyHex, userOpHash) {
  const account = privateKeyToAccount(privateKeyHex);
  ...
  return r + normalizedS + vHex;
}
function encodeAutonomousAgentSignature(agentAddress, ecdsaSig) {
...
function encodeEscalatedAgentSignature(agentAddress, ecdsaSig, passkeyIds, passkeySignatures) {
  if (ecdsaSig.length !== 132) {
    throw new Error("Invalid ECDSA signature length: expected 65 bytes");
  }
